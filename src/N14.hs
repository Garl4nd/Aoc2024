...
import Useful (groupByUnique, charGridToStr)
...
moveRobot dims steps robot@Robot{pos, vel} = let 
  pos' = pos |+| (steps |*| vel) in robot{pos = mod2 pos' dims}   
...
solution1 :: [Robot] -> Int 
solution1 robots = let 
  dims = (101, 103)
  movedRobots = moveRobot dims 100 <$> robots 
  quadrGroups = groupByUnique id $ classifyRobot dims  <$> movedRobots
  in product [num | (quadr, num) <- quadrGroups, quadr /= Center]

dims :: (Int, Int) = (101, 103)
printRobots :: [Robot] -> String
printRobots robots = let
  strAr = A.accumArray (const id) '.' ((0,0),  dims |+| (-1, -1)) [(( pos robot), 'x') | robot <- robots]   
...
solution2 :: [Robot] -> Int 
solution2 =  humanParser where 
  humanParser = const 8270
