...
parseFile file = foldr updateGraph M.empty $ lines file
 where
...
countNCliques n clique =
  let
    tNum = S.size $ S.filter (('t' ==) . head) clique
    cSize = S.size clique
   in
    if cSize < n || tNum == 0 then 0 else binom cSize n - binom (cSize - tNum) n
...
solution2 graph =
  let
   ...
   in
    intercalate "," . S.toList $ head cliques

getSolutions23 :: String -> IO (Int, Int)
getSolutions23 = const $ return (0, 0)
